// Credit: https://github.com/Fennec-hub/viewHelper/blob/main/src/ViewHelper.ts
import {
	BackSide,
	BufferAttribute,
	BufferGeometry,
...
const axesColors = [new Color(0x9a24fc), new Color(0x0069d2), new Color(0x26ee2c)];
...
class ViewHelper extends Object3D {
	...
	render() {
		...
		this.renderer.clearDepth(); // won't work without this line
...
function getAxesLines() {
	...
	const color = Array(6)
		.fill(0)
		.map((_, i) =>
			i < 2 ? axesColors[0].toArray() : i < 4 ? axesColors[1].toArray() : axesColors[2].toArray()
		)
		.flat();
...
function updatePointer(e: PointerEvent, domRect: DOMRect, orthoCamera: OrthographicCamera) {
...
function isClick(e: PointerEvent, startCoords: Vector2, threshold: number = 10) {
	return (
